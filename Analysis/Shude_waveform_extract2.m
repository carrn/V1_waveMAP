...
for j = 1:3 %length(matFiles)
    ...
    s = load(matFilename);
    fprintf('\n %d:',j);
    allWaveforms(j).templateID = s.templates.templateID;
    ...
    allWaveforms(j).spike_timing = s.templates.spike_timing;
    allWaveforms(j).spike_ID = s.templates.spike_templateID;
    allWaveforms(j).stim_on = s.templates.stimulus_onset;
    allWaveforms(j).dur = s.templates.stimulus_duration;
    allWaveforms(j).stim_cond = s.templates.stimulus_conditionID;
    

    %% Plotting waveforms
    [~,idx] = sort(allWaveforms(j).templateID,'descend');
    validChans = [1:384];
       
    normWaveforms(j).templateID = allWaveforms(j).templateID;
    
    Y = squeeze(allWaveforms(j).template(idx,validChans,:));
    Y = Y - repmat(nanmean(Y,2),[1 size(Y,2)]);
    ampls = max(Y,[],2)-min(Y,[],2);
    [~,maxLoc] = max(ampls);
    Y = Y(maxLoc,:);
    normWaveforms(j).template = Y;
%     normWaveforms(j).snr = s.templates.template_snr;
%     normWaveforms(j).depth = s.templates.template_depth_relativetolayer4;
%     normWaveforms(j).layerId = s.templates.template_layerID;
%     normWaveforms(j).spike_timing = s.templates.spike_timing;
%     normWaveforms(j).spike_ID = s.templates.spike_templateID;
%     normWaveforms(j).stim_on = s.templates.stimulus_onset;
%     normWaveforms(j).dur = s.templates.stimulus_duration;
%     normWaveforms(j).stim_cond = s.templates.stimulus_conditionID;
...
%% Sort out bad snr values, ie. low snrs or snr of -1 : too few corresponding spikes
        if allWaveforms(j).snr(i) == -1 | allWaveforms(j).snr(i) < 1 | abs(X(1)) > 45
            ...
            spike_timing = allWaveforms(j).spike_timing(idx(i));
            spike_ID = allWaveforms(j).spike_ID(idx(i));
            stim_on = allWaveforms(j).stim_on(idx(i));
            stim_cond = allWaveforms(j).stim_cond(idx(i));
...
%% normalized
    ...
    for n = 1:size(allWaveforms(j).norm_pos,1)
        e = allWaveforms(j).norm_pos(n,:)+allWaveforms(j).depth_pos(n);
        plot(e,'color',[0.2275    0.5961    0.8392])
