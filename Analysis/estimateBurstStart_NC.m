...
for sessionID =1:5        % per session
    fprintf('\n%d.', sessionID);  
    for clusterID = 1:5   % per cluster
        fprintf('%d',clusterID);
        templateIDpercluster = allWaveforms(sessionID).clusters(clusterID).ID;
        perUnitBurst = [];
        for unitID = 1:size(templateIDpercluster)  % per unit
            ...
            for trial=1:length(stimTimes) 
                % get the spike times for this unit for this trial
                ...
                % If this unit spiked for this trial, get the spike times
                % and detect if it burst
                    if ~isempty(currTrial(trial).spikes)
                        ...
                        if isempty(burst.T_start)
                            burstStart(trial) = NaN;
                        else
                            burstStart(trial) = burst.T_start(1);
                        end
                    else
                        burstStart(trial) = NaN;
                    end
            ...
            allWaveforms(sessionID).clusters(clusterID).perUnitBurst(unitID) = unitBurst;
