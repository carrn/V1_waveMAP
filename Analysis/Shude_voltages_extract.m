...
matFiles = dir('*_averaged_voltages.mat');
...
for j = 1:3 %length(matFiles)
    ...
    s = load(matFilename);
    fprintf('\n %d:',j);
    allWaveforms(j).template = s.avgWaveforms;
...
%% For each template - extract the maximum waveform
        X = allWaveforms(j).average(idx(i),validChans,:);
...
%% Sort out bad snr values, ie. low snrs or snr of -1 : too few corresponding spikes
        if allWaveforms(j).snr(i) == -1 | allWaveforms(j).snr(i) < 0.5 | abs(X(1)) > 45
            poor = [poor; X];
        else
            good = [good; X];
            depth = allWaveforms(j).depth(idx(i));
            ID = allWaveforms(j).templateID(idx(i));
            %hold on
            %plot(X)
...
ID_neg = cat(1,allWaveforms(1:3).ID_neg);
ID_pos = cat(1,allWaveforms(1:3).ID_pos);
...
save('ID_neg.mat', 'ID_neg');
save('ID_pos.mat', 'ID_pos');
