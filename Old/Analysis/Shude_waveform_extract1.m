...
%% Negative Spiking
            elseif t1 > t2 %if max peak occurs after min peak = neg spike
             ...
             %% FR, ISI for passing waveforms
             n = size(find(allWaveforms(j).spikeID == i)); % Number
             T = max(allWaveforms(j).spike_timing);        %
...
%% normalized
    ...
    for mm = 1:size(allWaveforms(j).norm_pos,1)
        d = allWaveforms(j).norm_pos(mm,:)+allWaveforms(j).depth_pos(mm);
