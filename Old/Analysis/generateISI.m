...
for clusterID =4%1:max(cluster)
    ...
    for sessionID = 1:5
        sessISIvalues = [];
        sessISIdensity = [];
        cluster_ID = allWaveforms(sessionID).clusters(clusterID).ID(allWaveforms(sessionID).clusters(clusterID).layerID==layerID);
        %cluster_ID = allWaveforms(sessionID).clusters(clusterID).ID;
        D = [0:0.0004:0.02];
        for unitID = 1:size(cluster_ID)
            
            if length(cluster_ID) > 7
            allSpikeTimes = allWaveforms(sessionID).spike_timing(allWaveforms(sessionID).spike_ID==[cluster_ID(unitID)]);
            stimTimes = allWaveforms(sessionID).stim_on;
