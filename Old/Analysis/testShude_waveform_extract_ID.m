...
for sessionID = 1:maxsession
    ...
    %% Plotting waveforms
    [~,idx] = sort(allWaveforms(sessionID).snr);
    validChans = [1:384];
    fig = figure;
...
%% normalize between 1 and -1
    ...
    %t = [0:41]; % Time samples at 30000 Hz, 42 samples
    subplot(2,1,1)
    hold on
    title(sprintf(['S',num2str(sessionID),' Waveforms',num2str(length(normalizedwaveformsneg)+length(normalizedwaveformspos))]))
